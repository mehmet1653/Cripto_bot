...
def yapay_zekayi_egit_ve_guncelle():
    ...
    try:
        ...
        print(f"🤖 Yapay zeka modeli (Inversed Strateji) güncellendi ve aktif! (Toplam Eğitim Verisi: {len(veriler)})", flush=True)
    except Exception as e:
...
def yapay_zeka_islem_onayi(rsi, adx, ema_fark, yon_kod, atr_yuzde, coin_id, derinlik_kod, hacim_orani, fiyat_degisim, sinyal_puani):
    ...
    try:
        ...
        dinamik_esik = 0.40 if sinyal_puani >= 90 else 0.45
...
@app.route('/')
def home():
    return f"Tersine Çevrilmiş (Breakout) Hibrit Bot Aktif | Aktif Pozisyon: {len(AKTIF_GRID_SISTEMLERI)}"
...
async def durum_komutu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        pozisyon_detaylari = ""
        if borsa_poslari:
            pozisyon_detaylari = "\n🔍 *Açık Pozisyonlar (Inversed / Trailing):*\n"
        ...
        mesaj = (
            f"🔄 *İNVERSED (TERS) MOMENTUM STRATEJİSİ*\n\n"
...
async def baslat_komutu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    BOT_CALISIYOR_MU = True
    await update.message.reply_text("🔄 *Tersine Çevrilmiş (Breakout) Bot Aktif Edildi!*", parse_mode='Markdown')
...
def otomatik_arkaplan_tarayici():
    ...
    print("🚀 [BAŞLANGIÇ] Inversed (Ters) Momentum & Breakout Stratejisi Devrede.", flush=True)
    try:
    ...
    while True:
        try:
            ...
            print(f"\n==================================================", flush=True)
            print(f"[{zaman_str}] 🔄 TERS STRATEJİ & YÖNETİM TURU (Tur #{tur_sayaci})", flush=True)
            print(f"==================================================", flush=True)
            ...
            # --- AÇIK POZİSYONLAR İÇİN TRAILING STOP ---
                ...
                if sym in aktif_borsa_map:
                    ...
                    try:
                        ...
                        if fiyat_farki_yuzde >= 0.8 and not kayitli_veri.get("trailing_aktif", False):
                            ...
                            hafizayi_kaydet()
                            print(f"🛡️ [TERS TRAILING AKTİF] {sym} kârda ilerledi, stop giriş fiyatına sabitlendi!", flush=True)
                            telegram_mesaj_gonder(f"🛡️ *KÂR KİLİTLENDİ (Inversed Trailing)*\n📌 `{sym}` pozisyonu kârda ilerlediği için stop seviyesi giriş fiyatına çekildi.")
                    except Exception as ex:
            ...
            for sym in list(AKTIF_GRID_SISTEMLERI.keys()):
                if sym not in aktif_borsa_map:
                    ...
                    if basarili_islem:
                        ...
                        telegram_mesaj_gonder(f"🎉 *Kâr Alındı (Inversed Hedef)*\n📌 Coindaki pozisyon başarıyla kapandı: `{sym}` 🟢")
                    else:
            ...
            # --- TERSİNE ÇEVRİLMİŞ (BREAKOUT) SİNYAL TARAMASI ---
            taranan_sinyaller = []
            su_anki_zaman = time.time()

            print(f"👀 Takip edilen {len(TAKIP_EDILENLER)} coin (Inversed Breakout mantığıyla) taraniyor...", flush=True)
            for symbol in TAKIP_EDILENLER:
                ...
                try:
                    ...
                    # 🔄 TERS MANTIK (BREAKOUT): Düşeni beklemek yok, yukarı koşan veya aşağı sert şelale olan yönün peşinden git!
                    long_onayli = (rsi >= 50) and (m_kapanis >= m_acilis or degisim_yuzdesi >= 0.5) and trend_1h_boga
                    short_onayli = (rsi <= 50) and (m_kapanis <= m_acilis or degisim_yuzdesi <= -0.5) and (not trend_1h_boga)
                ...
                if degisim_yuzdesi >= 1.0 and long_onayli:
                    grid_yonu = "LONG"
                    sinyal_puani = 88
                elif degisim_yuzdesi <= -1.0 and short_onayli:
                    grid_yonu = "SHORT"
                    sinyal_puani = 88
                elif long_onayli:
                    grid_yonu = "LONG"
            ...
            for sinyal in taranan_sinyaller:
                ...
                try:
                    ...
                    islem_tipi_str = f"🔄 Inversed Breakout (%2) + Trailing"

                    print(f"🚀 TERS İŞLEM AÇILDI: {symbol} | Yön: {grid_yonu} | Giriş: {giris_fiyati} | TP: {hedef_fiyat}", flush=True)
                    telegram_mesaj_gonder(
                        f"🔄 *İNVERSED (TERS) İŞLEM AÇILDI*\n\n"
                        f"📌 *Coin:* `{symbol}` | 📊 *Yön:* `{grid_yonu}`\n"
                        f"🎯 *İşlem Türü:* `{islem_tipi_str}`\n"
                        f"📈 *Sinyal Puanı:* `{sinyal_puani} / 100`\n"
                        f"⚙️ *Kaldıraç:* `{dinamik_kaldirac}x`\n"
                        f"🎯 *Hedef TP:* `+{hedef_roe:.0f}% ROE` (`{hedef_fiyat}`)\n"
