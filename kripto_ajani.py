...
# ==================== RİSK VE HİSTEREZİS PARAMETRELERİ ====================
ZAMAN_DILIMI = "1h"

# ADX Histerezis (Testere döngüsünü engellemek için ölü bölge)
ADX_ACIKLAMA_ESIK = 32.0  # İşlem açmak/sürdürmek için izin verilen maksimum ADX
ADX_KAPATMA_ESIK = 35.0   # Acil kapatma eşiği
...
def sinyal_uret(df, params):
    ...
    # Histerezis bazlı ADX Filtresi (Güçlü trendde mean reversion açma)
    if not pd.isna(son_adx) and son_adx > ADX_ACIKLAMA_ESIK:
        return None, f"ADX yüksek ({son_adx:.1f})"
    ...
    if son_fiyat <= son_alt and son_rsi < params['rsi_long'] and son_fiyat > son_open:
        stop_pct = max(MIN_STOP_PCT, min(MAX_STOP_PCT, (atr_pct * params['atr_stop_mult']) / 100.0))
        tp_pct = stop_pct * params['risk_reward'] + KOMISYON_ORANI
        return {
            "yon": "LONG", "giris": float(son_fiyat),
            "stop_pct": stop_pct, "tp_pct": tp_pct,
            "rsi": float(son_rsi), "atr_pct": atr_pct, "adx": float(son_adx)
        }, "OK"
...
def backtest_coin(symbol, params):
    try:
        df_raw = fetch_ohlcv_guvenli(symbol, ZAMAN_DILIMI, limit=1000)
...
async def durum_komutu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        mesaj = (
            ...
            f"🧠 Optimize: `{opt}/6`\n"
            f"⚙️ Zaman: `{ZAMAN_DILIMI}` | Kaldıraç: `{mod_al()['kaldirac']}x`\n\n"
...
def otomatik_arkaplan_tarayici():
    ...
    print(f"💰 {len(TAKIP_EDILENLER)} coin | Mod: {AKTIF_MOD} | Zaman: {ZAMAN_DILIMI}", flush=True)
        ...
        try:
            ...
            for symbol in TAKIP_EDILENLER:
                ...
                try:
                    ...
                    df_raw = fetch_ohlcv_guvenli(symbol, ZAMAN_DILIMI, limit=200)
