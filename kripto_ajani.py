...
# ==================== GRID PARAMETRELERİ ====================
GRID = {
    "aciklama": "📊 Grid Bot (Testere Piyasa)",
    "grid_aralik_pct": 0.02,      # ±%2
    "grid_sayisi": 10,             # 10 seviye
    "grid_kar_pct": 0.005,         # %0.5 her grid
    "kaldirac": 3,
    "toplam_yatirim_pct": 0.20,    # Kasanın %20'si
    "max_coin": 4,
    # Koruma
    "adx_dur_esik": 35,
    "fiyat_disi_pct": 0.05,       # ±%5 dışında kapat
    "gunluk_max_kayip_pct": 0.03,
    "min_hacim_usdt": 50_000_000,  # $50M
}

# ==================== DURUMLAR ====================
BOT_CALISIYOR_MU = True
AKTIF_GRIDLER = {}   # {symbol: grid_dict}
GUN_BASI_KASA = None
GUN_BASI_TARIH = None
ANALITIK = {
    "toplam_kar": 0.0,
...
def grid_olustur(symbol):
    ...
    try:
        ...
        for i in range(GRID['grid_sayisi']):
            seviye = alt + (adim * i)
            if seviye < fiyat * 0.998:  # Fiyatın altında
                try:
...
def grid_takip(symbol):
    """Grid emirlerini takip et, tetiklenenleri yenile."""
    ...
    try:
        ...
        # Tetiklenenleri kontrol et (SAT oldu mu?)
        tetiklenenler = [e for e in grid['emirler'] if e['durum'] == 'tetiklendi']
        for emir in tetiklenenler:
            if 'satis_id' in emir and str(emir['satis_id']) not in acik_id_set:
                # SAT tetiklendi → kâr realize
                ...
                try:
                    ...
                    print(f"💰 {symbol} | SAT tetiklendi → +{kar_usdt:.2f} USDT | Yeni AL @ {emir['fiyat']}", flush=True)
                except Exception as e:
...
@app.route('/')
def home():
    return f"Grid Bot | Aktif: {len(AKTIF_GRIDLER)} | Kâr: {ANALITIK.get('toplam_kar', 0):.2f}"
        ...
        if AKTIF_GRIDLER:
            ...
            for sym, g in AKTIF_GRIDLER.items():
                ...
                grid_detay += (
                    ...
                    f"  Açık: `{acik_sayisi}` | Tetik: `{tetiklenen}` | Kâr: `{g.get('toplam_kar', 0):+.2f}`\n"
                )
        else:
            grid_detay = "\n📊 *Aktif Grid Yok*\n"
        
        mesaj = (
            f"📊 *GRID BOT (Testere Piyasa)*\n\n"
    ...
    def run():
        ...
        for symbol in TAKIP_EDILENLER:
            ...
            if ok:
                ...
                time.sleep(2)
        if kurulan == 0:
            telegram_mesaj_gonder("⚠️ Hiçbir grid kurulamadı. Kasa veya hacim yetersiz olabilir.")
...
async def grid_detay_komutu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not AKTIF_GRIDLER:
        await update.message.reply_text("Aktif grid yok. `/grid_kur` yaz.", parse_mode='Markdown')
        return
    ...
    for sym, g in AKTIF_GRIDLER.items():
        ...
        satirlar.append(f"Toplam Kâr: `{g.get('toplam_kar', 0):+.2f}` USDT")
        satirlar.append(f"Emir sayısı: `{len(g['emirler'])}`")
        
        for e in g['emirler'][:5]:  # İlk 5
            durum = e['durum']
            if durum == 'acik':
                satirlar.append(f"  • AL @ `{e['fiyat']:.4f}` (bekliyor)")
            else:
...
def otomatik_arkaplan_tarayici():
    ...
    print(f"💰 {len(TAKIP_EDILENLER)} coin | Aralık: ±%{GRID['grid_aralik_pct']*100} | Kaldıraç: {GRID['kaldirac']}x", flush=True)
    ...
    while True:
        try:
            ...
            if dongu_sayaci % 20 == 0:
                print(f"🔍 #{dongu_sayaci} | Grid: {len(AKTIF_GRIDLER)} | Kâr: {ANALITIK.get('toplam_kar', 0):+.2f} | Tetik: {ANALITIK.get('toplam_grid_tetiklenme', 0)}", flush=True)
