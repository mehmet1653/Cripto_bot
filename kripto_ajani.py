        ...
        if genel_trend == "LONG":
            if ema9 > ema21 > ema50 and fiyat > ema21:
                if mod['rsi_long_min'] <= rsi <= mod['rsi_long_max']:
                    stop_pct = max(0.008, min(0.030, (atr_pct * mod['atr_stop_mult']) / 100.0))
                    return {"yon": "LONG", "giris": float(fiyat),
                            "stop": float(fiyat * (1 - stop_pct)),
                            "tp": float(fiyat * (1 + stop_pct * mod['risk_reward'])),
                            "stop_pct": stop_pct, "tp_pct": stop_pct * mod['risk_reward'],
                            "atr": float(atr), "rsi": float(rsi)}, "OK"
        elif genel_trend == "SHORT":
            if ema9 < ema21 < ema50 and fiyat < ema21:
                if mod['rsi_short_min'] <= rsi <= mod['rsi_short_max']:
                    stop_pct = max(0.008, min(0.030, (atr_pct * mod['atr_stop_mult']) / 100.0))
                    return {"yon": "SHORT", "giris": float(fiyat),
                            "stop": float(fiyat * (1 + stop_pct)),
                            "tp": float(fiyat * (1 - stop_pct * mod['risk_reward'])),
                            "stop_pct": stop_pct, "tp_pct": stop_pct * mod['risk_reward'],
                            "atr": float(atr), "rsi": float(rsi)}, "OK"
...
def grid_olustur(symbol, genel_trend):
    ...
    try:
            ...
            for i in range(mod['grid_sayisi']):
                ...
                if seviye < fiyat * 0.998:
                    try:
                        ...
                        emir = exchange.create_order(symbol, 'limit', 'buy', miktar,
            ...
            for i in range(mod['grid_sayisi']):
                ...
                if seviye > fiyat * 1.002:
                    try:
                        miktar = float(exchange.amount_to_precision(symbol, miktar_per_grid))
                        if miktar <= 0: continue
                        emir = exchange.create_order(symbol, 'limit', 'sell', miktar,
                            float(exchange.price_to_precision(symbol, seviye)))
        ...
        AKTIF_GRIDLER[symbol] = {
            ...
            "genel_trend": genel_trend, "fiyat_baslangic": fiyat,
            "emir_sayisi": len(emirler), "acilis_zaman": int(time.time()*1000),
            "toplam_kar": 0.0
        }
...
def backtest_trend(symbol, params, gun_sayisi=180):
    try:
        o4h = exchange.fetch_ohlcv(symbol, params['zaman_trend'], limit=min(gun_sayisi*6, 1000))
        ...
        for i in range(60, len(df1h)):
            ...
            if pd.isna(ema50) or pd.isna(ema200):
                continue
            fark = abs(ema50 - ema200) / ema200
            genel_trend = "BELIRSIZ" if fark < params['trend_belirsiz_pct'] else ("LONG" if ema50 > ema200 else "SHORT")
            bar = df1h.iloc[i]
...
def optimize_coin(symbol, gun_sayisi=180):
    ...
    if len(df4h) < 200 or len(df1h) < 200:
        return None
    rl_list = [(40, 60), (45, 65), (50, 70)]
    asm_list = [1.2, 1.5, 1.8]
    rr_list = [1.5, 2.0, 2.5]
    en_iyi = None
    for (rlm, rlx) in rl_list:
        for asm in asm_list:
            for rr in rr_list:
                params = dict(base_mod)
                params['rsi_long_min'] = rlm
                params['rsi_long_max'] = rlx
                params['rsi_short_min'] = 100 - rlx
                params['rsi_short_max'] = 100 - rlm
                params['atr_stop_mult'] = asm
                params['risk_reward'] = rr
                r = _backtest_with_data(df4h, df1h, params)
                if r and r.get('islem', 0) >= 5:
                    if en_iyi is None or r['pf'] > en_iyi['pf']:
                        en_iyi = {"params": params, "pf": r['pf'], "win_rate": r['win_rate'],
                                  "toplam": r['toplam'], "max_dd": r['max_dd'], "islem": r['islem']}
    return en_iyi

def _backtest_with_data(df4h, df1h, params):
    try:
        trades = []
        poz = None
        for i in range(60, len(df1h)):
...
async def temizle_komutu(update, context):
    global COIN_PARAMS, AKTIF_POZISYONLAR, AKTIF_GRIDLER
    COIN_PARAMS = {}
    AKTIF_POZISYONLAR = {}
    AKTIF_GRIDLER = {}
    hafizayi_kaydet()
    await update.message.reply_text("🗑️ *Temizlendi.*\nOptimize parametreleri silindi.", parse_mode='Markdown')
        ...
        try:
            ...
            if gunluk is not None:
                ...
                if gunluk >= MOD['gunluk_max_kar_pct']:
                    telegram_mesaj_gonder(f"✅ *Günlük kâr hedefi!*")
            ...
            for symbol in TAKIP_EDILENLER:
                ...
                if not uygun: continue
                genel_trend = GLOBAL_TREND.get(symbol, "BELIRSIZ")
                if genel_trend == "BELIRSIZ": continue
                ...
                if rejim == "TREND":
                    ...
                    if sig:
                        ...
                        kasa = float(bal['total'].get('USDT', 0))
                        if kasa < 10: continue
                        if not set_leverage_and_margin_safely(symbol, params['kaldirac']): continue
